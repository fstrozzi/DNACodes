package org.ptp
package DNABarcodes
	class HammingTen extends Generics with barcodeGenerator {
		...
		def generateBarcode(dna: String) : String = {
			...
			val p4 = generateParityBit(codeLength, Array(data(4)))
			val p5 = generateParityBit(codeLength, Array(p1,p2,data(0),p3,data(1),data(2),data(3),p4,data(4)))
			val hamming = Array(p1,p2,data(0),p3,data(1),data(2),data(3),p4,data(4),p5)
		...
		private def getParity(codeLength: Int, quadCode: Array[Int]) : Array[Int] = {
			...
			val p4 = calculateParityBit(codeLength, Array(quadCode(7),quadCode(8)))
